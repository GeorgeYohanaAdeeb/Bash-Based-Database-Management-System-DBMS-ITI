...
update_specific_column() {
    ...
    if [ -f "$table_name" ]; then
        ...
        column_name=$(whiptail --title "selecting column" --inputbox "Enter the name of the column to update:" --nocancel 20 60 3>&1 1>&2 2>&3)
...
update_specific_row() {
    ...
    if [ -f "$table_name" ]; then
                   id=$(whiptail --title "selecting record" --inputbox "Enter the ID of the record to update:" --nocancel 20 60 3>&1 1>&2 2>&3)
	...
	# Read the PK column name from the metadata file
        ...
        if [ -n "$id_column_index" ]; then
           ...
           # printf '%s\n' "${columns[@]}"
            ...
            if [ "$column_name" != "$primary_key_column" ]; then
                ...
                if [[ " ${columns[@]} " =~ " $column_name " ]]; then
                    ...
                    if awk -F: -v colindex="$column_index" -v value="$current_value" 'BEGIN{found=0} $colindex==value{found=1} END{exit !found}' "$table_name"; then
                        ...
                        if [[ "$datatype" == "integer" && "$new_value" =~ ^[0-9]+$ ]] || [[ "$datatype" == "string" && "$new_value" =~ ^[a-zA-Z]+$ ]]; then
                            # Update the row with the new value
			   awk -F: -v colindex="$column_index" -v value="$current_value" -v new_value="$new_value" -v id="$id" -v id_index=$id_column_index 'BEGIN{OFS=":"} { if ($id_index==id) gsub(value,new_value,$colindex); print $0;} ' "$table_name" > "$table_name.tmp" && mv "$table_name.tmp" "$table_name"
            ...
            whiptail --title "Error!" --msgbox "The specified ID does not exist in the table." 20 60
        fi
    else
        whiptail --title "Error!" --msgbox "Table '$table_name' does not exist." 20 60
    fi
...
table_name=$(whiptail --title "selecting table" --inputbox "Enter the name of the table:" --nocancel 20 60 3>&1 1>&2 2>&3)
